...
global quantum Fisher information (QFI) of a symmetric exponential pulse using the new set
...
#######################################################################################
@jit
def sym_exp_pulse(tval, tcap, avg_num_phot):
    """
    Parameters
    ----------
    ...
    avg_num_phot: non-negative number corresponding to the average number of
         ...
         (Note: int_{-inf}^{inf} dt sym_exp_pulse(t, tcap, num_phot)**2 = avg_num_phot)

    Returns
    -------
    real number, which corresponds to the amplitude of the symmetric exponential
    function
    """
    ### Starting time of the pulse is chosen so that it is zero upto four decimals
    thrs = 10.0**-8  # if you change this, make sure to change thrs in ODEs as well
    tcen = -0.5 * tcap * jnp.log(((thrs**2) * tcap) / avg_num_phot)

    pulse = jnp.sqrt(avg_num_phot / tcap) * (jnp.exp(-(jnp.abs(tval - tcen)) / tcap))
...
def ode_system(tval, rval, args):
    ...
    args: tuple consisting of two real numbers, (tcap, avg_num_phot)
          corresponding to T in the symm exp pulse shape(see paper) and
    ...
    ########################################################
    ### ODES for QFI:#######################################
    ftval = sym_exp_pulse(tval, tcap, avg_num_phot)
...
def qfi_odes_contribs(tcap, avg_num_phot):
    ...
    Parameters
    ----------
    tcap: real number corresponding to T in the symmetrix exp pulse shape(see paper)
    ...
    thrs = 10.0**-8  # if you change this, make sure to change in the pulse as well
    tcen = -0.5 * tcap * jnp.log(((thrs**2) * tcap) / avg_num_phot)
    tfinal = 2 * tcen + (10 / gamma_cap) * jnp.log(10)
...
def qfi_odes(tcap, avg_num_phot):
    ...
    Parameters
    ----------
    tcap: real number corresponding to T in the symmetric exp pulse shape(see paper)
    avg_num_phot: real number corresponding to the average number of
                  photons in the pulse
...
def coef_mat(tval, gamma1_cap, gamma2_cap, tcap, avg_num_phot):
    ...
    gamma2_cap: positive real number, right side coupling strength in the
                double-sided master equation
    tcap: real number corresponding to T in the sym exp pulse shape(see paper)
...
def fidelity_trace(tcap, avg_num_phot, epsilon):
    ...
    ### Final time for the symmetric exponential pulse
...
def objective_func_jax(tcap, avg_num_phot):
    ...
    Parameters
    ----------
    tcap: real number corresponding to T in the exp sym pulse shape(see paper)
    avg_num_phot: positive real number, average number of pump photons
...
def sing_inst_qfi_optimizer(ini_seed, avg_num_phot):
    ...
    solver = jaxopt.ScipyMinimize(
        fun=objective_func_jax, method="L-BFGS-B", maxiter=10000, tol=1e-6
    )
...
def fpval_analytic(tcap):
    ...
    Returns
    -------------
    analytic expression for Fp per unti photon with the exp sym pulse shape

    """

    return (4 * tcap * (4 + tcap)) / (2 + tcap) ** 2
